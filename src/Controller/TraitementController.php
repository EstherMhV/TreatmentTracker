...
class TraitementController extends AbstractController
{
    ...
    {
        $pumaScenarioNames = ['PUMA RESIDENCE', 'PUMA DGFIP', 'PUMA REGULARITE'];
        $totalVmCount = 0;
    
        foreach ($pumaScenarioNames as $scenarioName) {
            $scenario = $entityManager->getRepository(Scenario::class)->findOneBy(['nom_scenario' => $scenarioName]);
    ...
    public function createTraitment(UserInterface $user, Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        foreach ($selectedProcesses as $processId) {
            ...
            if ($process && $process->getIdScenario()->getId() == $idScenario) {
        ...
        $scenarioNames = ['PUMA RESIDENCE', 'PUMA DGFIP', 'PUMA REGULARITE'];

        if (in_array($traitement->getIdScenario()->getNomScenario(), $scenarioNames)) {
    ...
    public function editTraitement ($id, Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        $this->addFlash('error', 'Traitement modifié.');
    ...
    private function replanifyTreatment(EntityManagerInterface $entityManager, Traitement $parentTraitement): void
    {
        ...
        $idProcesses[] = $parentTraitement->getIdProcess();
        $idVms[] = $parentTraitement->getIdVm();
